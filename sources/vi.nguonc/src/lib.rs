//! # Nguồn C source (`vi.nguonc`)
//!
//! Scrapes the **NGUỒN C** REST API at `https://phim.nguonc.com` (the site
//! fronts as `nguonc.com`, which redirects to `phim.nguonc.com`). The site
//! documents its own JSON API on `/api-document`:
//!
...
//!
//! The detail API does not hand out media URLs: each episode is an **embed
//! iframe** (`https://embed{N}.streamc.xyz/embed.php?hash=<hex>`) whose player
//! (JWPlayer + Cloudflare Turnstile "verification" layer) resolves the real
//! playlist. `get_stream` replays the player's own two POSTs against the embed
//! endpoint:
//!
...
//!
//! ## Data model mapping to Komorei (mirrors `vi.ophim`)
//!
...
// ────────────────────────────────────────────────────────────────────────────
// JSON structs — nguonc list/detail envelopes.
// ────────────────────────────────────────────────────────────────────────────
...
/// URL. `referrer` is the nguonc film page the embed would be loaded from;
...
impl ListingProvider for NguoncSource {
...
impl DynamicListings for NguoncSource {
...
impl DynamicFilters for NguoncSource {
	fn get_dynamic_filters(&self) -> Result<Vec<Filter>> {
		Ok(vec![
			...
			.into(),
			Filter::note(
				"Nguồn dữ liệu từ Nguồn C (https://phim.nguonc.com). Link phát được cấp quyền theo tập qua máy chủ embed.",
			),
		])
	}
}
...
impl DynamicSettings for NguoncSource {
	fn get_dynamic_settings(&self) -> Result<Vec<Setting>> {
		Ok(vec![
			TextSetting {
				key: SETTING_BASE_URL.into(),
				title: "Địa chỉ API Nguồn C".into(),
...
impl NotificationHandler for NguoncSource {
...
impl DeepLinkHandler for NguoncSource {
...
// ────────────────────────────────────────────────────────────────────────────
// Migration — nguonc ids are stable slugs, keep identity.
// ────────────────────────────────────────────────────────────────────────────

impl MigrationHandler for NguoncSource {
